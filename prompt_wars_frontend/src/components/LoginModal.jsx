...
export default function LoginModal({ isOpen, onClose, onLogin }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // Dispatch verification email payload to backend / auth service
      try {
        ...
        console.log('Local API notification dispatched');
      }

      setLoading(false);
      sound.playSuccess();
      
      // Exact message banner requested by user
      setConfirmationNotice(`Account created. Check your inbox to confirm your email, then sign in.`);
      setActiveTab('signin');
    } catch (err) {
      setLoading(false);
      setErrorMsg('Failed to dispatch confirmation email. Please try again.');
    }
  };
  ...
  const handleGoogleAuth = () => {
    sound.playClick();
    const cleanEmail = email.trim() || 'preetlassipeele@gmail.com';
    const usernameHandle = cleanEmail.split('@')[0];

    const userProfile = {
      id: `google_${Date.now()}`,
      username: usernameHandle,
      name: fullName.trim() || 'Preet Lassi',
      email: cleanEmail,
      ...
      skills: [{ name: 'Python', level: 5 }, { name: 'FastAPI', level: 5 }, { name: 'React', level: 4 }],
